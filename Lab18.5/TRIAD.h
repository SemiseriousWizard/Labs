...
public:
	...
	bool operator > (const TRIAD& triad) {
		...
		return (first > triad.first || (first == triad.first && 
			(second > triad.second || (second == triad.second && third > triad.third))));
	}
	bool operator >= (const TRIAD& triad) {
		return (first > triad.first || (first == triad.first && 
			(second > triad.second || (second == triad.second && third >= triad.third))));
	...
	bool operator <= (const TRIAD& triad) {
		/*return ((first <= triad.first) && 
		(second <= triad.second) && (third <= triad.third));*/
		return (first < triad.first || (first == triad.first && 
			(second < triad.second || (second == triad.second && third <= triad.third))));
	}
};